...
def test_conf_name_for_platforms(tmp_path: Path):
    ...
    write_project_files(
        tmp_path,
        build_kodpm_json("17.0", "fincomtech", [], image="registry.example.com/fincomtech:17"),
...
def test_compose_adds_missing_addon_option(tmp_path: Path):
    write_project_files(tmp_path, build_odoo_json(), build_user_settings("base"))
    project = ProjectFiles(tmp_path)
    values = build_values(project, "local")
    values.setdefault("config", {}).setdefault("options", {})["server_wide_modules"] = (
        "base,web,queue_job"
    )
    project.conf_path.write_text(render_conf_from_values(values).replace(
        "server_wide_modules = base,web,queue_job\n", ""
    ), encoding="utf-8")
    raw = compose_conf(project, values)
    assert get_ini_option(raw, "server_wide_modules") == "base,web,queue_job"
    assert get_ini_option(raw, "workers") == "0"


def build_odoo_json():
    return build_kodpm_json("17.0", "odoo", [])
